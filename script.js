...
const STATE_CACHE_URL = "Cache/homepage-state.json";
...
async function loadCachedState() {
  try {
    const response = await fetch(`${STATE_CACHE_URL}?t=${Date.now()}`, { cache: "no-store" });
...
async function saveCachedState() {
  ...
  try {
    if (globalThis.chrome?.runtime?.sendMessage) {
      await chrome.runtime.sendMessage({ type: "homepage-save-state", state });
      return;
    }
  } catch {
  ...
  try {
    ...
    console.error("Не удалось сохранить состояние хомпейджа:", error);
  }
}
